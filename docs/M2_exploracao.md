...
A análise foi conduzida através de uma abordagem complementar, combinando a observação da correlação linear com a variável alvo, a comparação dos valores médios entre classes e a análise visual das distribuições através de boxplots. Esta abordagem permite não só identificar associações estatísticas, mas também compreender a capacidade discriminativa de cada variável.

Os resultados evidenciam que a variável `SatisfactionIndex` apresenta a associação mais relevante com `Attrition` (correlação ≈ -0.17). Verifica-se que colaboradores com níveis mais elevados de satisfação global tendem a apresentar menor probabilidade de abandono da organização, sendo esta diferença também visível na separação das distribuições entre as classes. Esta relação é consistente com a interpretação dos indicadores individuais de satisfação e reforça a importância desta dimensão no contexto da retenção de talento, sugerindo que fatores relacionados com bem-estar e equilíbrio profissional desempenham um papel determinante na decisão de permanência.

A variável `RatioYearsInRole` apresenta igualmente uma associação negativa moderada (≈ -0.13), indicando que colaboradores com maior proporção do tempo passado na função atual tendem a permanecer na organização. Este resultado sugere que a estabilidade funcional, ou uma progressão interna consistente, pode estar associada a níveis mais elevados de retenção. Este indicador acrescenta valor analítico ao capturar padrões de mobilidade interna que não são diretamente observáveis através das variáveis originais.

Relativamente à variável `IncomePerLevel`, observa-se também uma associação negativa (≈ -0.11), evidenciando que colaboradores com menor rendimento relativo ao nível hierárquico apresentam maior propensão para abandonar a empresa. A análise das distribuições confirma esta tendência, indicando que a perceção de desajuste entre compensação e responsabilidade pode constituir um fator relevante na decisão de saída. Este resultado reforça a importância de políticas salariais equilibradas no contexto da gestão de recursos humanos.

Por outro lado, a variável `CareerStagnation` apresenta uma associação mais fraca com a variável alvo (≈ -0.08), não evidenciando um padrão claro de diferenciação entre colaboradores que permanecem e os que abandonam a organização. Apesar de conceptualmente relevante, este resultado sugere que a operacionalização da variável poderá não capturar de forma adequada o fenómeno de estagnação na carreira, ou que este fator, isoladamente, não é suficientemente discriminativo no contexto do dataset. Esta limitação poderá justificar uma eventual reformulação da variável em análises futuras.

De forma global, as novas variáveis demonstram capacidade para capturar dimensões relevantes do comportamento organizacional, nomeadamente satisfação profissional, estabilidade funcional e adequação entre remuneração e posição hierárquica. A sua inclusão contribui para enriquecer o conjunto de atributos disponíveis, podendo melhorar a capacidade preditiva dos modelos a desenvolver nas fases subsequentes.
