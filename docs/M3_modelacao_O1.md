...
A tabela seguinte apresenta os resultados obtidos com a divisão 65/35 com estratificação por `Attrition_bin`, que produziu os melhores resultados globais de todos os *splits* testados (65/35, 70/30, 75/25, 80/20, 85/15, 90/10, com e sem *stratify*). A estratificação garante a mesma proporção de colaboradores com atrito (~16%) em ambos os conjuntos, sendo uma prática obrigatória em datasets desequilibrados (Géron, 2022; James et al., 2021). O modelo vencedor foi a Regressão Logística (Candidato 8), selecionada com base em dois critérios formais: F1 Teste e interpretabilidade. A Regressão Logística obteve o F1 Teste mais elevado de todos os 18 candidatos (0.5538), combinado com um AUC de 0.8236 e o menor *overfitting* do grupo linear (+0.12). Embora o FT-Transformer tenha alcançado um AUC Teste superior (0.8449), foi preterido por ser um modelo de caixa-negra, sem capacidade de interpretação direta dos coeficientes — um requisito relevante no contexto de Recursos Humanos, onde a identificação dos fatores que contribuem para o atrito é tão importante quanto a previsão em si (Hom et al., 2017). A Regressão Logística é, por isso, o único modelo que combina desempenho preditivo competitivo, *overfitting* controlado e interpretabilidade dos resultados (James et al., 2021; Géron, 2022).
