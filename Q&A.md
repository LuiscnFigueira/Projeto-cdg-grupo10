...
### "Os limiares do índice de risco - 30%, 50%, 70% - são arbitrários. Como os justificam?"

**Resposta:** Não são completamente arbitrários, mas também não são derivados matematicamente - e é importante ser honesto sobre isso. O limiar de 50% respeita o ponto de corte natural da Regressão Logística: é o valor a partir do qual o modelo classifica um colaborador como provável saída. Esse limiar não foi inventado - resultou da otimização do threshold em 0,79, que define quem o modelo alerta. Os limiares de 30% e 70% foram definidos para criar categorias com significado operacional: abaixo de 30% o risco é baixo o suficiente para não requerer ação imediata; acima de 70% a probabilidade é suficientemente alta para justificar intervenção prioritária. Num contexto real, estes limiares seriam calibrados com o próprio departamento de RH, com base na sua capacidade de intervenção e nas consequências de cada tipo de erro.
